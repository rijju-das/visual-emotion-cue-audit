...
def aggregate(rows: List[Dict]) -> Dict:
    ...
    result = {"n_pairs": len(eligible), "n_control_pairs": len(controls), "cue_coverage": cue_coverage, "directional_success_rate": response, "emotion_js_divergence_mean": mean("emotion_js_divergence"), "va_distance_mean": mean("va_distance"), "feature_cosine_mean": mean("feature_cosine"), "conflict_uncertainty_success_rate": uncertainty, "original_ece": ece, "original_brier_mean": mean("original_brier"), "original_nll_mean": mean("original_nll"), "original_va_mae": float(np.mean([0.5 * (row["original_valence_absolute_error"] + row["original_arousal_absolute_error"]) for row in unique_originals if not math.isnan(row["original_valence_absolute_error"])])), "cause_diagnostic_score": cause, "cause_note": "Unvalidated diagnostic composite: response, content preservation, cue grounding (or coverage when VLM is disabled), conflict uncertainty, and calibration.", "by_cue": by_cue, "matched_control_analysis": paired_by_cue}
